        ...
        **/
        function generateRazorpayOrderForm()
        {
            ...
            if (isset($this->keyID) && isset($this->keySecret) && $amount!=null)
            {
				...
				$_SESSION['rzp_QP_description'] = $description;
        ...
        function wpCheckRazorpayResponse()
        {
            ...
            if (!empty($attributes))
            {
                 $amount = $_SESSION['rzp_QP_amount'] ; // paise to rupees

				 $description = $_SESSION['rzp_QP_description'] ; 
                ...
                if ($success === true)
                {
                    ...
                    . "<br><br>" . "Transaction ID : " . esc_html($attributes['razorpay_payment_id']) . "<br><br>" . "Order Amount: ₹".$amount/100;
...
if(($amount/100) == 649){
	...
	date_default_timezone_set('Asia/Kolkata');
                    $a = date('Y-m-d');
                    $expiration = date('Y-m-d H:i:s',strtotime('-0 year +3 months -0 days',strtotime($a)));
	
}else if (($amount/100) == 999){
	$plan_type = 2;
	date_default_timezone_set('Asia/Kolkata');
                    $a = date('Y-m-d');
                    $expiration = date('Y-m-d H:i:s',strtotime('-0 year +6 months -0 days',strtotime($a)));
}else if(($amount/100) == 4999){
	$plan_type = 3;
					$a = date('Y-m-d');
                    $expiration = date('Y-m-d H:i:s',strtotime('+1 year -0 months -0 days',strtotime($a)));
}
					else if($description == 'lawfirm'){
	$plan_type = 4;
					$a = date('Y-m-d');
                    $expiration = date('Y-m-d H:i:s',strtotime('+1 year -0 months -0 days',strtotime($a)));
}
					else if($description == 'lawschools'){
	$plan_type = 5;
					$a = date('Y-m-d');
                    $expiration = date('Y-m-d H:i:s',strtotime('+1 year -0 months -0 days',strtotime($a)));
}
...
$query = "SELECT * FROM membership WHERE user_id LIKE ".get_current_user_id() ;
...
$query = "UPDATE membership SET expiration = '".$expiration."', name = ". $amount/100 .", plan_type = ". $plan_type ." WHERE user_id = ".get_current_user_id() ;
...
$query = "INSERT INTO membership (expiration,name,user_id,plan_type) VALUES ('" .$expiration. "'," .($amount/100). "," .get_current_user_id(). "," .$plan_type. ")"; 
